...
from ..api.models import VoiceInteraction
...
class VoiceAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "voice_agent", config: dict[str, Any] | None = None):
        ...
        try:
            ...
            if not audio_data:
                yield AgentMessage(
                    ...
                    from_agent=self.agent_id,
                    to_agent=message.from_agent,
                    content={"type": "error", "error": "No audio data provided"},
                    timestamp=datetime.now(),
                )
                return
            ...
            if not processed_audio:
                yield AgentMessage(
                    ...
                    from_agent=self.agent_id,
                    to_agent=message.from_agent,
                    content={"type": "error", "error": "Audio preprocessing failed"},
                    timestamp=datetime.now(),
                )
                return
            ...
            response_text = await self._generate_response(transcription["text"], user_id)
            ...
            await self._save_interaction(user_id, transcription, response_text, tts_result)
    ...
    async def _preprocess_audio(self, audio_data: bytes, user_id: str) -> bytes | None:
        """
    ...
    async def _generate_response(self, query_text: str, user_id: str) -> str:
        """
        Generate response using MAS system
        """
        try:
    ...
    async def _route_to_analytics_agent(self, query: str, user_id: str) -> str | None:
        """
    ...
    async def _route_to_search_agent(self, query: str, user_id: str) -> str | None:
        """
    ...
    async def _route_to_reporting_agent(self, query: str, user_id: str) -> str | None:
        """
    ...
    async def _route_to_arbiter_agent(self, query: str, user_id: str) -> str | None:
        """
        ...
        try:
            ...
            if self.current_interaction:
                yield AgentMessage(
                    ...
                    from_agent=self.agent_id,
                    to_agent=message.from_agent,
                    content={"type": "error", "error": "Voice session already active"},
                    timestamp=datetime.now(),
                )
                return
    ...
    async def _end_voice_session(self, message: AgentMessage) -> AsyncGenerator[AgentMessage, None]:
        """
        ...
        try:
            ...
            if not self.current_interaction:
                yield AgentMessage(
                    ...
                    from_agent=self.agent_id,
                    to_agent=message.from_agent,
                    content={"type": "error", "error": "No active voice session"},
                    timestamp=datetime.now(),
                )
                return
            ...
            session_duration = (
                datetime.now() - self.current_interaction["start_time"]
            ).total_seconds()
        ...
        try:
            ...
            if command == "pause":
                ...
                status = {
                    ...
                    "session_id": (
                        self.current_interaction.get("session_id")
                        if self.current_interaction
                        else None
                    ),
                    "user_id": (
                        self.current_interaction.get("user_id")
                        if self.current_interaction
                        else None
                    ),
                    "session_duration": (
                        (datetime.now() - self.interaction_start_time).total_seconds()
                ...
                yield AgentMessage(
                    ...
                    from_agent=self.agent_id,
                    to_agent=message.from_agent,
                    content={"type": "error", "error": f"Unknown voice command: {command}"},
                    timestamp=datetime.now(),
                )
    ...
    async def _save_session_summary(self, session_data: dict[str, Any]):
        """
    ...
    async def run_continuous_session(self, user_id: str, max_duration: int = 300):
        """
                    ...
                    async for response in self.process_message(message):
                        if response.content.get("type") == "voice_response":
                            ...
                            if audio_path and os.path.exists(audio_path):
                                ...
                                logger.info(
                                    f"Playing response: {response.content['response']['text'][:50]}..."
                                )
...
if __name__ == "__main__":
    ...
    async def test_voice_agent():
        ...
        test_message = AgentMessage(
            ...
            content={"type": "start_voice_session", "user_id": "test_user"},
            timestamp=datetime.now(),
        )
