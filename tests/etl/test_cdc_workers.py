...
def test_health_check_all_healthy(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(cluster=SimpleNamespace(health=lambda: {"status": "green"})),
    )
    ...
    monkeypatch.setattr(
        cw,
        "get_qdrant_client",
        lambda: SimpleNamespace(get_collections=lambda: SimpleNamespace(collections=[1, 2])),
    )
...
def test_health_check_degraded_when_pg_fails(monkeypatch):
    # pg raises
    monkeypatch.setattr(
        cw, "get_pg_connection", lambda: (_ for _ in ()).throw(Exception("conn fail"))
    )
    ...
    monkeypatch.setattr(
        cw,
        "get_qdrant_client",
        lambda: SimpleNamespace(get_collections=lambda: SimpleNamespace(collections=[1])),
    )
    monkeypatch.setattr(cw, "get_redis_client", lambda: SimpleNamespace(ping=lambda: True))
