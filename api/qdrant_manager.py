...
class QdrantManager:
    ...
    def __init__(
        ...
        logger.info(f"Qdrant client initialized: {self.host}:{self.port}, collection: {self.collection_name}")
    ...
    def create_collection(
        ...
        try:
            ...
            logger.info(f"Created collection: {self.collection_name}, vector_size={vector_size}, distance={distance}")
            return True
            ...
            for i in range(0, len(points), batch_size):
                ...
                if qdrant_points:
                    ...
                    if result.status == UpdateStatus.COMPLETED:
                        ...
                        logger.debug(f"Upserted batch {i//batch_size + 1}: {len(qdrant_points)} points")
                    else:
                        stats["failed"] += len(qdrant_points)
                        logger.error(f"Upsert failed for batch {i//batch_size + 1}: {result.status}")
