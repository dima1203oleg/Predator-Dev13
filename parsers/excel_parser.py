...
class ExcelParser:
    ...
    def __init__(
        ...
        self.required_columns = [
            "hs_code", "date", "amount", "qty", "country_code",
    ...
    def parse(
        ...
        if file_ext == ".csv":
            ...
            df_iterator = pd.read_excel(file_path, chunksize=self.chunk_size)
        else:
    ...
    def _build_record(self, row: pd.Series, chunk_idx: int, row_idx: int) -> Dict[str, Any]:
        ...
        record = {
            ...
            "country_code": str(row["country_code"]).strip().upper() if pd.notna(row["country_code"]) else None,
            ...
            "customs_office": str(row["customs_office"]).strip() if pd.notna(row["customs_office"]) else None,
        }
