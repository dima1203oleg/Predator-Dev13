...
from ..api.models import ChaosExperiment, ResilienceTest, FailureInjection
...
class ChaosAgent(BaseAgent):
    ...
    async def start(self):
        ...
        self.experiment_scheduler_task = asyncio.create_task(self._continuous_experiment_scheduler())
    ...
    async def _check_experiment_safety(
        ...
        try:
            ...
            if experiment_type in dangerous_experiments and not self.chaos_config["experiments"][experiment_type]["enabled"]:
                return {
                    "safe": False,
            ...
            for target in targets:
                try:
                    ...
                    matching_pods = [
                        ...
                        if pod_pattern in pod.metadata.name
                    ]
...
"""
            ...
            for target in targets:
                try:
                    ...
                    stress_cmd = [
                        ...
                        "stress-ng",
                        "--cpu", str(cpu_percentage // 10),  # stress-ng uses number of CPUs
                        "--vm", "1",
                        "--vm-bytes", f"{memory_percentage}%",
                ...
                for target in targets:
                    try:
                        ...
                        metrics[target] = {
                            ...
                            "ready_containers": sum(1 for c in pod_metrics.status.container_statuses or [] if c.ready),
    ...
    async def _collect_post_experiment_metrics(
        self,
        targets: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    async def _cleanup_experiment(
        ...
        try:
            ...
            if experiment_type == "network_delay":
                ...
                if self.chaos_config["tools"]["litmus_chaos"]["enabled"]:
                    cleanup_cmd = [
                        "kubectl", "delete", "chaosengine",
                        f"network-delay-{experiment_id[:8]}",
                        "-n", self.chaos_config["tools"]["litmus_chaos"]["namespace"]
                    ]
