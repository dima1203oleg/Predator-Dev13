...
"""
    ...
    def _query_llm(self, model_name: str, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if ":" in model_name or model_name.startswith("llama") or model_name.startswith("gemma"):
    ...
    def _calculate_consensus(self, votes: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "votes": [
                {
                    "model": v["model"],
                    "answer_preview": v["response"].get("answer", "")[:100]
                }
                for v in votes
            ],
...
if __name__ == "__main__":
    ...
    ctx = AgentContext(
        user_id="test",
        query="Чи є ризики у цій компанії?",
        session_id="test",
        trace_id="test"
    )
    
    # Mock previous results
    ctx.results["Retriever"] = {
        "total_count": 150,
        "merged": [{"company_name": "TestCorp"}]
    }
    ctx.results["Miner"] = {
