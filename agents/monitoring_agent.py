...
from ..api.models import MonitoringMetrics, AlertHistory, SystemHealth
...
class MonitoringAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.monitoring_config = {
            ...
            "grafana": self.config.get("grafana", {
            ...
            "alerting": self.config.get("alerting", {
                "rules": {
                    ...
                    "response_time": {"threshold": 2000, "duration": "5m", "severity": "warning"}
                },
                "channels": {
                    "email": {"enabled": True, "recipients": ["admin@predator-analytics.com"]},
                    "slack": {"enabled": True, "webhook_url": self.config.get("slack_webhook")},
                    "telegram": {"enabled": True, "bot_token": self.config.get("telegram_bot_token"), "chat_id": self.config.get("telegram_chat_id")},
        ...
        self.monitoring_state = {
            "metrics_history": deque(maxlen=self.monitoring_config["processing_options"]["max_metrics_history"]),
            "active_alerts": {},
            ...
            try:
                if self.monitoring_config["alerting"]["channels"]["slack"]["enabled"]:
                    self.slack_client = slack.WebClient(
                        token=self.monitoring_config["alerting"]["channels"]["slack"].get("bot_token")
                    )
            ...
            try:
                if self.monitoring_config["alerting"]["channels"]["telegram"]["enabled"]:
                    self.telegram_client = telegram.Bot(
                        token=self.monitoring_config["alerting"]["channels"]["telegram"]["bot_token"]
                    )
        ...
        try:
            ...
            if cpu_usage > cpu_threshold:
                alerts.append({
                    ...
                    "type": "cpu_usage",
                    "severity": self.monitoring_config["alerting"]["rules"]["cpu_usage"]["severity"],
            ...
            memory_threshold = self.monitoring_config["alerting"]["rules"]["memory_usage"]["threshold"]
            
            if memory_usage > memory_threshold:
                alerts.append({
                    "alert_id": str(uuid.uuid4()),
                    "type": "memory_usage",
                    "severity": self.monitoring_config["alerting"]["rules"]["memory_usage"]["severity"],
            ...
            if disk_usage > disk_threshold:
                alerts.append({
                    "alert_id": str(uuid.uuid4()),
                    "type": "disk_usage",
                    "severity": self.monitoring_config["alerting"]["rules"]["disk_usage"]["severity"],
            ...
            if error_rate > error_threshold:
                alerts.append({
                    "alert_id": str(uuid.uuid4()),
                    "type": "error_rate",
                    "severity": self.monitoring_config["alerting"]["rules"]["error_rate"]["severity"],
    ...
    async def _send_alert(self, alert: Dict[str, Any]):
        ...
        try:
            ...
            # Send via Slack
            if self.monitoring_config["alerting"]["channels"]["slack"]["enabled"] and self.slack_client:
                await self._send_slack_alert(alert, alert_message)
            
            # Send via Telegram
            if self.monitoring_config["alerting"]["channels"]["telegram"]["enabled"] and self.telegram_client:
                await self._send_telegram_alert(alert, alert_message)
            
            # Send via SMS
            if self.monitoring_config["alerting"]["channels"]["sms"]["enabled"] and self.twilio_client:
    ...
    async def _send_email_alert(self, alert: Dict[str, Any], message: str):
        ...
        try:
            msg = MIMEMultipart()
            msg['From'] = self.monitoring_config["alerting"]["channels"]["email"].get("from", "alerts@predator-analytics.com")
            msg['To'] = ', '.join(self.monitoring_config["alerting"]["channels"]["email"]["recipients"])
            msg['Subject'] = f"Predator Analytics Alert: {alert['type']}"
            
            msg.attach(MIMEText(message, 'plain'))
            
            server = smtplib.SMTP(
                self.monitoring_config["alerting"]["channels"]["email"].get("smtp_server", "localhost"),
        ...
        try:
            ...
            if not metrics or "insights_generated" in metrics:
                panels.append({
                    ...
                    "type": "graph",
                    "targets": [{
                        "expr": "insights_generated_total",
                        "legendFormat": "Insights"
                    }],
                    "yAxes": [{"unit": "none"}]
                })
            ...
            while True:
                try:
                    ...
                    for endpoint in endpoints:
                        await self._perform_health_check(f"http://localhost:8000{endpoint}", "basic")
