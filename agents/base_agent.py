...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        if self.enable_metrics:
            self.execution_counter = Counter(
                ...
                f"Total executions of {name} agent",
                ["status"]
            )
            self.execution_duration = Histogram(
                f"agent_{name}_duration_seconds",
                f"Execution duration of {name} agent"
            )
            self.active_executions = Gauge(
                f"agent_{name}_active",
                f"Active executions of {name} agent"
            )
    ...
    def get_model(self, model_type: str = "primary") -> Optional[str]:
