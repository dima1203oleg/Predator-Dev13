...
class DocumentationAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.doc_config = {
            ...
            "quality_assurance": self.config.get(
                "quality_assurance",
                {
                    "spell_check": True,
                    "grammar_check": True,
    ...
    async def _generate_documentation(
        self, doc_type: str, source_files: list[str], output_format: str, include_examples: bool
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if doc_type == "code_docs":
                doc_content = await self._generate_code_documentation(
                    source_analysis, include_examples
                )
                ...
                if stripped.startswith('"""') or stripped.startswith("'''"):
                    if in_docstring:
                        ...
                        docstring_lines.append(
                            stripped[3:] if stripped.startswith('"""') else stripped[3:]
                        )
                elif in_docstring:
    ...
    async def _generate_code_documentation(
        self, source_analysis: dict[str, Any], include_examples: bool
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            # Module overview
            modules_section = {"title": "Modules", "content": []}
            ...
            for module in source_analysis.get("modules", []):
                module_doc = {
                    ...
                    "functions_count": len(
                        [
                            f
                            for f in source_analysis.get("functions", [])
                            if f.get("module") == module.get("name")
                        ]
                    ),
                    "classes_count": len(
                        [
                            c
                            for c in source_analysis.get("classes", [])
                            if c.get("module") == module.get("name")
                        ]
                    ),
                }
            ...
            functions_section = {"title": "Functions", "content": []}
            ...
            for func in source_analysis.get("functions", []):
                func_doc = {
                    ...
                    "examples": (
                        await self._generate_function_examples(func) if include_examples else []
                    ),
                }
            ...
            classes_section = {"title": "Classes", "content": []}
            ...
            for cls in source_analysis.get("classes", []):
                class_doc = {
                    ...
                    "examples": (
                        await self._generate_class_examples(cls) if include_examples else []
                    ),
                }
    ...
    async def _generate_general_documentation(
        self, source_analysis: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    async def _format_documentation(self, doc_content: dict[str, Any], output_format: str) -> str:
        """
    ...
    async def _generate_doc_metadata(
        self, doc_type: str, source_files: list[str], generation_start: datetime
    ) -> dict[str, Any]:
        """
    ...
    async def _generate_function_examples(self, func: dict[str, Any]) -> list[str]:
        ...
        try:
            ...
            if args:
                examples.append(
                    f"result = {func_name}({', '.join([f'{arg}=None' for arg in args])})"
                )
