...
class IntegrationAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.integration_config = {
            "api_management": self.config.get(
                "api_management",
                {
                    "rate_limiting": True,
                    "authentication": True,
            ...
            "system_integrations": self.config.get(
                "system_integrations",
                {
                    "external_apis": True,
                    "webhooks": True,
    ...
    async def start(self):
        ...
        self.http_session = aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(
                total=self.integration_config["processing"]["timeout_seconds"]
            )
        )
    ...
    async def _handle_api_call(self, message: AgentMessage) -> AsyncGenerator[AgentMessage, None]:
        """
    ...
    async def _execute_api_call(
        ...
        try:
            ...
            if not await self._check_circuit_breaker(endpoint):
                return {"success": False, "error": "Circuit breaker open", "endpoint": endpoint}
    ...
    async def _add_authentication(self, request_config: dict[str, Any], endpoint: str):
        """
    ...
    async def _get_oauth2_token(self, endpoint: str) -> str | None:
        """
    ...
    async def _check_circuit_breaker(self, endpoint: str) -> bool:
        ...
        try:
            breaker = self.circuit_breakers.get(
                endpoint,
                {"failures": 0, "last_failure": None, "state": "closed"},  # closed, open, half_open
            )
    ...
    async def _record_circuit_breaker_failure(self, endpoint: str):
        ...
        try:
            breaker = self.circuit_breakers.get(
                endpoint, {"failures": 0, "last_failure": None, "state": "closed"}
            )
    ...
    async def _handle_data_flow(self, message: AgentMessage) -> AsyncGenerator[AgentMessage, None]:
        """
    ...
    async def _orchestrate_data_flow(
        self, flow_id: str, input_data: dict[str, Any], flow_config: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    async def _execute_flow_step(
        self, step_config: dict[str, Any], input_data: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    async def _execute_api_step(
        ...
        try:
            ...
            if api_result.get("success"):
                ...
                return {"success": False, "error": api_result.get("error"), "output": api_result}
    ...
    async def _execute_transformation_step(
        ...
        try:
            ...
            if transformation_type == "json_path":
                return await self._execute_json_path_transformation(
                    transformation_config, input_data
                )

            elif transformation_type == "template":
                return await self._execute_template_transformation(
                    transformation_config, input_data
                )
    ...
    async def _execute_validation_step(
        ...
        try:
            ...
            return {
                "success": True,
                "output": {"valid": all_valid, "validation_results": validation_results},
            }
