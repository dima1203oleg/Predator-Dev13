...
class SentimentAnalyzerAgent(BaseAgent):
    ...
    def __init__(
        ...
        # Background tasks (commented out for request-response model)
        # self.model_update_task = None
        # self.language_detection_task = None
        # self.pattern_analysis_task = None

        logger.info(f"Sentiment Analyzer Agent initialized: {name}")

    async def initialize(self):
        """
        Initialize the agent with required models and NLP tools.
        This method should be called once before the agent starts processing requests.
        """
        await self._initialize_nlp_tools()
        await self._initialize_sentiment_models()
        await self._initialize_transformer_models()
        logger.info("Sentiment Analyzer Agent models and tools initialized.")

    async def _execute_impl(self, context: AgentContext) -> dict[str, Any]:
        """
        Agent implementation for sentiment analysis and NLP processing.
        ...
        if message_type == "analyze_sentiment":
            ...
            return await self._perform_emotion_detection(context)
        elif message_type == "analyze_text":
            return await self._perform_text_analysis(context)
        elif message_type == "aspect_sentiment":
            return await self._perform_aspect_sentiment(context)
        elif message_type == "linguistic_analysis":
            return await self._perform_linguistic_analysis(context)
        elif message_type == "sentiment_trends":
            return await self._perform_sentiment_trends(context)
        elif message_type == "text_processing":
            return await self._perform_text_processing(context)
        else:
        ...
        try:
            ...
            for lang, model_name in spacy_models.items():
                try:
                    ...
                    logger.warning(f"spaCy model {model_name} not available or failed to download")
                except Exception as e:
    ...
    async def _initialize_sentiment_models(self):
        ...
        (Note: These models are instantiated but not trained. Training data and process would be needed for full functionality.)
        """
        try:
            ...
            self.emotion_models = {
                "emotion_rf": RandomForestClassifier(n_estimators=100, random_state=42),
                "emotion_xgb": xgb.XGBClassifier(n_estimators=100, random_state=42),
            }
            ...
            logger.info("Sentiment models initialized (untrained)")
    ...
    # Background tasks are commented out as this agent is refactored to a request-response model.
    # If continuous background processing is needed, an external orchestration service should manage these.
    ...
    async def _perform_sentiment_analysis(self, context: AgentContext) -> dict[str, Any]:
        """
        Perform sentiment analysis request
        """
        try:
            text = context.metadata.get("text", "")
            method = context.metadata.get("method", "ensemble")
            language = context.metadata.get("language", "auto")
            ...
            self.sentiment_history[language].append(
                {"text_hash": text_hash, "sentiment": sentiment_result, "timestamp": datetime.now()}
            )
                    ...
                    if scaler and hasattr(scaler, 'transform'): # Check if scaler is fitted
                        ...
                        predictions[model_name] = {
                            ...
                            "probabilities": {
                                cls: float(prob)
                                for cls, prob in zip(
                                    self.sentiment_config["sentiment_classes"], probs
                                )
                            },
    ...
    def _extract_sentiment_features(self, text: str) -> list[float] | None:
        """
                ...
                if "sentiment" in model_name.lower():
                    try:
                        ...
                        for pred in (
                            ...
                            if isinstance(pred, dict):
                                ...
                                if "positive" in label or "pos" in label:
                                    if score > 0.5:
                                        sentiment_scores[
                                            "positive" if score > 0.7 else "very_positive"
                                        ] = score
                                    else:
                                    ...
                                    if score > 0.5:
                                        sentiment_scores[
                                            "negative" if score > 0.7 else "very_negative"
                                        ] = score
                                    else:
                                        sentiment_scores["neutral"] = score
                                else:
                        ...
                        results[model_name] = {
                            ...
                            "top_sentiment": (
                                max(sentiment_scores.items(), key=lambda x: x[1])
                                if sentiment_scores
                                else ("neutral", 0.5)
                            ),
                        }
    ...
    def _calculate_ensemble_sentiment(self, results: dict[str, dict]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "confidence": float(avg_confidence),
                "method_agreement": (
                    sentiment_votes[ensemble_sentiment] / len(results) if results else 0
                ),
                "components": results,
            }
    ...
    async def _perform_emotion_detection(self, context: AgentContext) -> dict[str, Any]:
        """
        Perform emotion detection request
        """
        try:
            text = context.metadata.get("text", "")

            if not text:
                raise ValueError("Text content required for emotion detection")
    ...
    async def _perform_text_analysis(self, context: AgentContext) -> dict[str, Any]:
        """
        Perform comprehensive text analysis request
        """
        try:
            text = context.metadata.get("text", "")

            if not text:
                raise ValueError("Text content required for text analysis")
    ...
    async def _perform_aspect_sentiment(self, context: AgentContext) -> dict[str, Any]:
        """
        Perform aspect-based sentiment analysis
        """
        try:
            text = context.metadata.get("text", "")
            aspects = context.metadata.get("aspects", [])

            if not text:
                raise ValueError("Text content required for aspect sentiment analysis")
            ...
            for aspect in aspects:
                ...
                if aspect_sentiments:
                    ...
                    aspect_results[aspect] = {
                        ...
                        "polarity": float(avg_polarity),
                        "confidence": float(
                            np.mean([s.get("confidence", 0.5) for s in aspect_sentiments])
                        ),
    ...
    def _calculate_overall_aspect_sentiment(
        self, aspect_results: dict[str, dict]
    ) -> dict[str, Any]:
        """
    ...
    async def _perform_linguistic_analysis(self, context: AgentContext) -> dict[str, Any]:
        """
        Perform linguistic analysis request
        """
        try:
            text = context.metadata.get("text", "")
    ...
    def _calculate_lexical_richness(self, text: str) -> float:
        ...
        # unique_words / total_words # Original TTR
    ...
    def _analyze_sentence_variety(self, text: str) -> dict[str, Any]:
        ...
        return {
            ...
            "variety_score": (
                float(1 / (1 + np.std(lengths) / np.mean(lengths))) if np.mean(lengths) > 0 else 0.0
            ),
        }
    ...
    def _detect_figurative_language(self, text: str) -> list[str]:
        ...
        # Hyperbole detection
        if any(
            word in text_lower for word in ["amazing", "incredible", "unbelievable", "fantastic"]
        ):
    ...
    async def _perform_sentiment_trends(self, context: AgentContext) -> dict[str, Any]:
        """
        Perform sentiment trends analysis request
        """
        try:
            time_window = context.metadata.get("time_window", 3600)  # seconds
            language = context.metadata.get("language", "all")
        ...
        try:
            ...
            for lang, sentiments in self.sentiment_history.items():
                if language == "all" or lang == language:
                    recent_sentiments = [
                        s for s in sentiments if s.get("timestamp", datetime.min) > cutoff_time
                    ]
    ...
    async def _perform_text_processing(self, context: AgentContext) -> dict[str, Any]:
        """
        Perform text processing request
        """
        try:
            text = context.metadata.get("text", "")
            operations = context.metadata.get("operations", ["normalize"])

            if not text:
                raise ValueError("Text content required for text processing")
...
if __name__ == "__main__":
    ...
    async def test_sentiment_analyzer_agent():
        ...
        print("Testing sentiment analyzer agent (sentiment analysis)...")
        try:
            response = await agent.execute(ctx_sentiment_analysis)
            print(f"Sentiment analysis response: {response.get('type')}")
            if response.get("type") == "sentiment_analysis_response":
                sentiment = response.get("sentiment", {})
                print(
                    f"Sentiment: {sentiment.get('sentiment')}, Polarity: {sentiment.get('polarity', 'N/A'):.3f}"
                )
        except Exception as e:
            print(f"Sentiment analysis failed: {e}")

        # Test context for emotion detection
        ctx_emotion_detection = AgentContext(
            user_id="test_user",
            query="detect emotions",
            session_id="test_session",
            trace_id="test_trace",
            metadata={
                "type": "detect_emotion",
        ...
        print("\nTesting sentiment analyzer agent (emotion detection)...")
        try:
            response = await agent.execute(ctx_emotion_detection)
            print(f"Emotion detection response: {response.get('type')}")
            if response.get("type") == "emotion_detection_response":
                emotions = response.get("emotions", {})
                print(f"Primary emotion: {emotions.get('primary_emotion', 'N/A')}")
        except Exception as e:
            print(f"Emotion detection failed: {e}")

        # Test context for text analysis
        ctx_text_analysis = AgentContext(
            user_id="test_user",
        ...
        try:
            response = await agent.execute(ctx_text_analysis)
            print(f"Text analysis response: {response.get('type')}")
            if response.get("type") == "text_analysis_response":
                analysis = response.get("analysis", {})
                print(f"Basic stats: {analysis.get('basic_stats')}")
                print(f"Sentiment: {analysis.get('sentiment', {}).get('sentiment')}")
        except Exception as e:
