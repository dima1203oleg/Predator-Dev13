...
from ..api.models import ImageAnalysis, ObjectDetection, OCRResult
...
class ImageProcessingAgent(BaseAgent):
    ...
    def __init__(
        self,
        agent_id: str = "image_processing_agent",
        ...
        self.image_config = {
            ...
            "processing_methods": self.config.get("methods", [
                "classification", "object_detection", "ocr", "face_recognition",
                "image_similarity", "segmentation", "enhancement", "analysis"
            ]),
        ...
        try:
            ...
            yield AgentMessage(
                ...
                content={
                    ...
                    "top_k": top_k,
                    "confidence_threshold": self.image_config["quality_thresholds"]["classification_confidence"]
                },
                timestamp=datetime.now()
            )
            ...
            if recognition_mode == "detect":
                ...
                for i, (face_encoding, face_location) in enumerate(zip(face_encodings, face_locations)):
                    ...
                    matches = face_recognition.compare_faces(
                        ...
                        face_encoding,
                        tolerance=self.image_config["quality_thresholds"]["face_recognition_threshold"]
                    )
    ...
    async def _handle_image_similarity(
        ...
        try:
            ...
            similarity_method = message.content.get("method", "embedding")  # embedding, perceptual, hash
    ...
    async def _compare_images(
        self,
        image_data_1: Optional[str] = None,
        image_data_2: Optional[str] = None,
        ...
        try:
            ...
            similarity_result = {
                ...
                "method": similarity_method,
                "is_similar": similarity_score >= self.image_config["quality_thresholds"]["similarity_threshold"],
                "processing_time": datetime.now()
            }
    ...
    async def _handle_image_enhancement(
        ...
        try:
            image_data = message.content.get("image_data")
            image_url = message.content.get("image_url")
            enhancement_type = message.content.get("enhancement_type", "auto")  # auto, sharpen, denoise, contrast
    ...
    async def _handle_image_analysis(
        ...
        try:
            image_data = message.content.get("image_data")
            image_url = message.content.get("image_url")
            analysis_types = message.content.get("analysis_types", ["classification", "objects", "text"])
    ...
    async def _analyze_image(
        ...
        try:
            ...
            if "classification" in analysis_types:
                analysis_results["classification"] = await self._classify_image(image_data, image_url, top_k=3)
