...
from ..api.models import ModelMetrics, DriftDetection, ModelTraining
...
class SelfImprovementAgent(BaseAgent):
    ...
    def __init__(
        self,
        agent_id: str = "self_improvement_agent",
        ...
        self.drift_config = {
            ...
            "retraining_trigger_threshold": self.config.get("retraining_trigger_threshold", 0.15),  # 15% degradation
    ...
    async def _monitor_model_performance(self, model_id: str):
        ...
        try:
            ...
            self.performance_history[model_id] = [
                entry for entry in self.performance_history[model_id]
    ...
    def _calculate_performance_metrics(self, prediction_data: List[Dict[str, Any]]) -> Dict[str, float]:
        """
    ...
    async def _handle_drift_detection(self, model_id: str, current_metrics: Dict[str, float]):
        ...
        try:
            ...
            if degradation_level >= self.drift_config["retraining_trigger_threshold"]:
                await self._trigger_model_retraining(model_id, f"Performance degradation: {degradation_level:.2%}")
            
        except Exception as e:
            logger.error(f"Drift detection handling failed: {e}")
    
    async def _assess_degradation_level(self, model_id: str, current_metrics: Dict[str, float]) -> float:
        """
        ...
        try:
            ...
            async for response in self.lora_trainer.process_message(training_message):
                if response.content.get("type") == "training_started":
                    ...
                    logger.info(f"Retraining started for model {model_id}, training ID: {training_id}")
                    ...
                    logger.error(f"Retraining failed to start for model {model_id}: {response.content.get('error')}")
                ...
                if older_metrics:
                    ...
                    if degradation > self.drift_config["retraining_trigger_threshold"]:
                        await self._trigger_model_retraining(
                            model_id, 
                            f"Gradual performance degradation: {degradation:.2%}"
                        )
        ...
        try:
            ...
            for model_id, model_config in self.monitored_models.items():
                ...
                if not last_updated or last_updated < cutoff_date:
                    await self._trigger_model_retraining(
                        model_id,
                        f"Model age exceeds {max_age_days} days"
                    )
    ...
    async def _handle_performance_analysis(
        ...
        try:
            ...
            analysis = await self._analyze_performance_trends_detailed(model_id, analysis_period_days)
        ...
        try:
            ...
            for metric_name in ["f1_score", "precision", "recall", "accuracy"]:
                ...
                if values:
                    ...
                    if np.mean(values) > 0:
                        analysis["volatility"][metric_name] = float(np.std(values) / np.mean(values))
