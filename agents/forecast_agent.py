...
class ForecastAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "forecast_agent", config: dict[str, Any] | None = None):
    ...
    async def _handle_forecast_request(
        ...
        try:
            ...
            forecast_result = await self._generate_forecast(
                historical_data, target_variable, forecast_periods, model_type
            )

            # Save forecast to database
            forecast_id = await self._save_forecast_result(
                target_variable, filters, forecast_result, message.content.get("user_id")
            )
    ...
    async def _get_historical_data(
        self, target_variable: str, filters: dict[str, Any]
    ) -> pd.DataFrame:
        """
        ...
        try:
            async with get_db_session() as session:
                # Build query
                query = session.query(
                    CustomsData.declaration_date, getattr(CustomsData, target_variable)
                ).filter(
    ...
    async def _generate_forecast(
        self, historical_data: pd.DataFrame, target_variable: str, periods: int, model_type: str
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                model_type == "ensemble"
                and "prophet" in forecast_result
                and "lightgbm" in forecast_result
            ):
                ensemble_forecast = self._create_ensemble_forecast(
                    forecast_result["prophet"], forecast_result["lightgbm"]
                )
            ...
            forecast_result["forecast_quality"] = self._calculate_forecast_quality(
                historical_data[target_variable].values, forecast_result["forecast_values"]
            )
    ...
    async def _forecast_with_prophet(
        self, data: pd.DataFrame, target_variable: str, periods: int
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            return {
                ...
                "performance": {
                    ...
                    "mape": np.mean(
                        np.abs(
                            (prophet_data["y"] - historical_forecast["yhat"]) / prophet_data["y"]
                        )
                    )
                    * 100,
                },
    ...
    def _create_ensemble_forecast(
        self, prophet_forecast: dict[str, Any], lgb_forecast: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "weights": {"prophet": prophet_weight, "lightgbm": lgb_weight},
            }
    ...
    def _calculate_forecast_quality(
        self, historical_values: np.ndarray, forecast_values: list[float]
    ) -> dict[str, Any]:
        """
        Calculate forecast quality metrics
        """
        ...
        try:
            ...
            if not forecast_id:
                yield AgentMessage(
                    ...
                    from_agent=self.agent_id,
                    to_agent=message.from_agent,
                    content={"type": "error", "error": "forecast_id required"},
                    timestamp=datetime.now(),
                )
                return
            ...
            async with get_db_session() as session:
                ...
                if not forecast:
                    yield AgentMessage(
                        ...
                        from_agent=self.agent_id,
                        to_agent=message.from_agent,
                        content={"type": "error", "error": "Forecast not found"},
                        timestamp=datetime.now(),
                    )
                    return
                ...
                evaluation = self._evaluate_forecast_accuracy(
                    json.loads(forecast.forecast_data), actual_data
                )
    ...
    async def _get_actual_vs_forecast(
        self, target_variable: str, filters: dict[str, Any], forecast_date: datetime.date
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            async with get_db_session() as session:
                ...
                return [{"date": row[0].isoformat(), "actual_value": float(row[1])} for row in rows]
    ...
    def _evaluate_forecast_accuracy(
        self, forecast_data: dict[str, Any], actual_data: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            mape = (
                np.mean(
                    np.abs(np.array(matched_actuals) - np.array(matched_forecasts))
            ...
            return {
                ...
                "matched_dates": list(zip(forecast_dates, forecast_values, matched_actuals))[
                    :10
    ...
    async def _retrain_models(self, target_variable: str, filters: dict[str, Any]) -> bool:
        """
    ...
    async def _generate_backtest_forecasts(
        self, target_variable: str, filters: dict[str, Any], backtest_periods: int
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            for i in range(backtest_periods):
                ...
                forecast = await self._generate_forecast(train_data, target_variable, 1, "ensemble")
                        ...
                        backtest_results.append(
                            {
                                ...
                                "percentage_error": abs(forecast_value - actual_value)
                                / actual_value
                                * 100,
                            }
                        )
