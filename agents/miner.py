...
class MinerAgent(BaseAgent):
        ...
        return [
            {
                ...
                "check": lambda record, median_price: record.get("price_per_kg", 999) < median_price * 0.5,
                "severity": "high"
            },
            {
            ...
            },
            {
                ...
                "check": lambda record, _: record.get("hs_code", "").startswith("42") and record.get("price_per_kg", 0) < 10,
                "severity": "medium"
            }
        ]
    ...
    def _check_corruption_patterns(self, records: List[Dict]) -> List[Dict[str, Any]]:
        ...
        return sorted(flags, key=lambda x: {"critical": 3, "high": 2, "medium": 1}.get(x["severity"], 0), reverse=True)
    ...
    def _generate_insights(
        ...
        if stats.get("top_companies"):
            ...
            insights.append(
                f"🏆 Лідер: {top['name']} ({top['amount']:,.0f} USD)"
            )
...
if __name__ == "__main__":
    ...
    mock_records = [
        ...
        {"pk": "2", "amount": 5000000, "qty": 1000, "company_name": "CompanyB", "hs_code": "8418"},  # Anomaly
