...
from ..api.models import DataQualityCheck, DataQualityMetric, DataQualityRule
...
class DataQualityAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.dq_config = {
            "quality_dimensions": self.config.get("quality_dimensions", [
                "accuracy", "completeness", "consistency", "timeliness", "validity", "uniqueness"
            ]),
            "validation_rules": self.config.get("validation_rules", [
                "range_check", "format_check", "consistency_check", "completeness_check",
                "uniqueness_check", "referential_integrity", "business_rules"
            ]),
            "data_types": self.config.get("data_types", [
                "numeric", "categorical", "text", "date", "boolean", "json"
            ]),
            ...
            "anomaly_detection": self.config.get("anomaly_detection", [
                "statistical", "ml_based", "rule_based", "time_series"
            ]),
            "profiling_methods": self.config.get("profiling_methods", [
                "basic_stats", "distribution_analysis", "correlation_analysis", "outlier_detection"
            ]),
            "quality_limits": self.config.get("quality_limits", {
    ...
    async def _handle_data_validation(
        ...
        try:
            ...
            # Validate data
            validation_result = await self._validate_data(data_source, validation_rules, validation_type)
    ...
    async def _validate_data(
        ...
        try:
            ...
            validation_result["invalid_records"] = len(set(error["record_index"] for error in all_errors))
            validation_result["valid_records"] = validation_result["total_records"] - validation_result["invalid_records"]
            
            # Calculate quality score
            validation_result["quality_score"] = await self._calculate_quality_score(validation_result)
    ...
    async def _apply_validation_rule(self, df: pd.DataFrame, rule: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _validate_range(self, df: pd.DataFrame, column: str, rule: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _validate_format(self, df: pd.DataFrame, column: str, rule: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _validate_categorical(self, df: pd.DataFrame, column: str, rule: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _validate_uniqueness(self, df: pd.DataFrame, columns: List[str], rule: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _validate_completeness(self, df: pd.DataFrame, column: str, rule: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _handle_quality_assessment(
        ...
        try:
            ...
            # Assess data quality
            assessment_result = await self._assess_data_quality(data_source, assessment_type, dimensions)
    ...
    async def _assess_data_quality(
        ...
        try:
            ...
            assessment_result["overall_score"] = round(sum(scores) / len(scores), 4) if scores else 0.0
            
            # Generate recommendations
            assessment_result["recommendations"] = await self._generate_quality_recommendations(dimension_scores)
    ...
    async def _assess_completeness(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_completeness = sum(stats["completeness_ratio"] for stats in completeness_stats.values()) / len(completeness_stats)
    ...
    async def _assess_consistency(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            for col in numeric_columns:
                total_checks += 1
                if df[col].std() > df[col].mean() * 10:  # High variance might indicate inconsistency
                    consistency_issues += 1
    ...
    async def _generate_quality_recommendations(self, dimension_scores: Dict[str, Any]) -> List[str]:
        """
            ...
            for dimension, score_info in dimension_scores.items():
                ...
                if score < threshold:
                    if dimension == "completeness":
                        ...
                        recommendations.append("Review data collection processes to reduce null values")
    ...
    async def _handle_anomaly_detection(
        ...
        try:
            ...
            # Detect anomalies
            anomaly_result = await self._detect_anomalies(data_source, detection_method, sensitivity)
    ...
    async def _statistical_anomaly_detection(self, df: pd.DataFrame, sensitivity: float) -> List[Dict[str, Any]]:
        """
    ...
    async def _ml_anomaly_detection(self, df: pd.DataFrame, sensitivity: float) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not numeric_data.empty:
                ...
                if self.anomaly_detectors["isolation_forest"]:
                    predictions = self.anomaly_detectors["isolation_forest"].fit_predict(scaled_data)
    ...
    async def _isolation_forest_detection(self, df: pd.DataFrame, sensitivity: float) -> List[Dict[str, Any]]:
        """
    ...
    async def _profile_data(self, data_source: Dict[str, Any], profiling_type: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Quality insights
            profile_result["quality_insights"] = await self._generate_quality_insights(df, column_profiles)
        ...
        try:
            ...
            if column_data.dtype in ['int64', 'float64']:
                profile.update({
                    ...
                    "mean": round(float(column_data.mean()), 4) if not column_data.empty else None,
                    "median": round(float(column_data.median()), 4) if not column_data.empty else None,
                    "std": round(float(column_data.std()), 4) if not column_data.empty else None,
                    "quartiles": {
    ...
    async def _generate_quality_insights(self, df: pd.DataFrame, column_profiles: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for column, profile in column_profiles.items():
                ...
                if profile.get("unique_percentage", 100) < 5 and profile.get("unique_count", 0) < 20:
    ...
    async def _handle_rule_checking(
        ...
        try:
            ...
            yield AgentMessage(
                ...
                to_agent=message.from_agent,
                content={
                    "type": "rule_checking_response",
                    "result": rule_check_result
                },
                timestamp=datetime.now()
            )
    ...
    async def _check_rules(self, data_source: Dict[str, Any], rules: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    async def _evaluate_python_expression(self, df: pd.DataFrame, expression: str) -> Dict[str, Any]:
        """
    ...
    async def _handle_quality_report(
        ...
        try:
            ...
            report_result = await self._generate_quality_report(data_source_id, report_type, time_range)
    ...
    async def _generate_quality_report(
        ...
        try:
            ...
            if report_type == "comprehensive":
                ...
                if data_source_id:
                    report["data_sources"][data_source_id] = await self._get_data_source_report(data_source_id, start_date)
    ...
    async def _get_quality_trends(self, start_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_logs = [log for log in self.quality_state["quality_log"] 
    ...
    async def _get_alerts_summary(self, start_date: datetime) -> Dict[str, Any]:
        ...
        """
        try:
            alerts = [alert for alert in self.quality_state["alerts"] 
    ...
    async def _generate_report_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            for source_id, source_data in report.get("data_sources", {}).items():
                ...
                if score < 0.8:
                    recommendations.append(f"Improve data quality for {source_id} (current score: {score})")
    ...
    async def _handle_quality_monitoring(
        ...
        try:
            ...
            yield AgentMessage(
                ...
                to_agent=message.from_agent,
                content={
                    "type": "quality_monitoring_response",
    ...
    async def test_data_quality_agent():
        ...
        validation_message = AgentMessage(
            ...
            content={
                ...
                "validation_rules": [
                    {"name": "age_range", "type": "range_check", "column": "age", "min_value": 0, "max_value": 120},
                    {"name": "email_format", "type": "format_check", "column": "email", "pattern_name": "email"},
                    {"name": "completeness", "type": "completeness_check", "column": "name", "threshold": 0.8}
                ],
