...
from ..api.models import RiskAssessment, RiskFactors, RiskAlerts
...
class RiskAssessmentAgent(BaseAgent):
    ...
    def __init__(
        self,
        agent_id: str = "risk_assessment_agent",
        ...
        self.risk_config = {
            "risk_dimensions": self.config.get("risk_dimensions", [
    ...
    async def _assess_entity_risk(self, entity: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    def _rule_based_risk_assessment(self, dimension: str, factors: Dict[str, Any]) -> Tuple[float, float]:
        """
    ...
    def _normalize_financial_factor(self, factor_name: str, value: Any) -> float:
        ...
        try:
            if factor_name == "liquidity_ratio":
                ...
                rating_map = {"AAA": 0, "AA": 0.1, "A": 0.2, "BBB": 0.4, "BB": 0.6, "B": 0.8, "CCC": 1.0}
    ...
    async def _evaluate_risk_alerts(self, assessment: Dict[str, Any]):
        ...
        try:
            ...
            elif risk_level == "high" and overall_score >= self.risk_config["alert_thresholds"]["high"]:
                alert_triggered = True
            ...
            for entity_id, history in self.assessment_history.items():
                if len(history) >= 5:  # Need sufficient history
                    ...
                    if len(recent_scores) >= 2:
                        ...
                        if abs(trend) > 0.05:
                            logger.info(f"Risk trend for {entity_id}: {trend_direction} ({trend:.3f})")
        ...
        try:
            ...
            for alert in self.risk_alerts:
                if alert["status"] == "active":
                    ...
                    entity_id = alert["entity_id"]
                    latest_assessment = self.assessment_history[entity_id][-1] if self.assessment_history[entity_id] else None
        ...
        try:
            ...
            if dimension:
                ...
                if dimension in latest["dimension_scores"]:
                    ...
                    analysis["dimension_analysis"]["factor_importance"] = self._analyze_factor_importance(
    ...
    def _analyze_factor_importance(self, dimension: str, factors: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if not dimension or dimension not in self.risk_config["risk_dimensions"]:
                yield AgentMessage(
                    ...
                    content={
                        "type": "error",
                        "error": "Valid dimension required"
                    },
                    timestamp=datetime.now()
                )
                return
    ...
    async def _update_dimension_models(self, dimension: str, training_data: List[Dict[str, Any]]) -> bool:
        """
        ...
        try:
            ...
            if report_type == "comprehensive":
                report.update({
                    ...
                    "risk_trends": self._calculate_risk_trends(history) if len(history) >= 3 else None,
                    "key_risk_factors": await self._identify_key_risk_factors(latest),
                    "recommendations": latest["recommendations"],
                    "alerts": [alert for alert in self.risk_alerts if alert["entity_id"] == entity_id][-5:]
                })
                ...
                report.update({
                    ...
                    "trend": self._calculate_risk_trends(history)["direction"] if len(history) >= 3 else "unknown"
                })
