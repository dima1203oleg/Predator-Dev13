...
from ..api.models import FraudDetection, FraudAlerts, TransactionPatterns
...
class FraudDetectionAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.fraud_config = {
            "detection_methods": self.config.get("detection_methods", [
                "isolation_forest", "random_forest", "xgboost", "lightgbm",
                "behavioral_analysis", "network_analysis", "rule_based"
            ]),
            "contamination": self.config.get("contamination", 0.01),  # Expected fraud ratio
            "alert_threshold": self.config.get("alert_threshold", 0.8),  # Fraud probability threshold
            "retraining_frequency": self.config.get("retraining_frequency", 3600),  # seconds
            "behavioral_window": self.config.get("behavioral_window", 30),  # days for behavioral analysis
    ...
    def _initialize_fraud_rules(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "condition": lambda tx: tx.get("amount", 0) % 1000 == 0 and tx.get("amount", 0) > 5000,
                "severity": "low",
    ...
    async def _analyze_transaction_fraud(self, transaction: Dict[str, Any]) -> float:
        ...
        try:
            ...
            ensemble_score = sum(
                fraud_scores.get(method, 0) * weight
    ...
    def _extract_transaction_features(self, transaction: Dict[str, Any]) -> Optional[List[float]]:
        """
    ...
    def _rule_based_fraud_detection(self, transaction: Dict[str, Any]) -> float:
        ...
        try:
            ...
            total_weight = sum(severity_weights.get(rule["severity"], 0.5) for rule in triggered_rules)
    ...
    def _check_transaction_frequency(self, transaction: Dict[str, Any]) -> bool:
        ...
        recent_txs = [tx for tx in recent_txs if (datetime.now() - tx.get("timestamp", datetime.now())).seconds < 3600]  # Last hour
    ...
    async def _behavioral_fraud_detection(self, transaction: Dict[str, Any]) -> float:
        ...
        try:
            ...
            amount_zscore = abs((amount - avg_amount) / max(profile.get("std_transaction_amount", 1), 1))
            ...
            # Frequency anomaly
            recent_txs = [tx for tx in self.transaction_history.get(user_id, []) 
    ...
    async def _network_fraud_detection(self, transaction: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if self.transaction_graph.has_node(user_id) and self.transaction_graph.has_node(merchant):
    ...
    async def _handle_fraud_alert(self, transaction: Dict[str, Any], fraud_score: float):
        ...
        try:
            alert = {
                ...
                "severity": "high" if fraud_score > 0.9 else "medium" if fraud_score > 0.8 else "low",
            ...
            logger.warning(f"Fraud alert generated for transaction {transaction.get('transaction_id')}: score={fraud_score:.3f}")
    ...
    async def _update_transaction_status(self, transaction: Dict[str, Any], fraud_score: float):
        ...
        try:
            ...
            logger.info(f"Transaction {transaction.get('transaction_id')} status updated to: {status}")
    ...
    async def _check_fraud_patterns(self):
        ...
        try:
            ...
            for user_transactions in self.transaction_history.values():
                recent = [tx for tx in user_transactions 
    ...
    async def _analyze_network_patterns(self):
        ...
        try:
            ...
            communities = list(community.greedy_modularity_communities(self.transaction_graph.to_undirected()))
    ...
    async def _analyze_user_behavior(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                "preferred_hours": df["timestamp"].dt.hour.mode().tolist() if "timestamp" in df.columns else [],
                "amount_volatility": df["amount"].std() / df["amount"].mean() if df["amount"].mean() > 0 else 0,
                "analyzed_at": datetime.now()
            }
