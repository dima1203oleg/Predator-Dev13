...
class SupportAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "support_agent", config: dict[str, Any] | None = None):
        super().__init__(agent_id, config or {})

        # Support configuration
        self.support_config = {
            "support_channels": self.config.get(
                "support_channels",
                {
                    "chat": True,
                    "email": True,
            ...
            "query_handling": self.config.get(
                "query_handling",
                {
            ...
            "knowledge_base": self.config.get(
                "knowledge_base",
                {
                    "faq_database": True,
            ...
            "multilingual_support": self.config.get(
    ...
    async def _handle_user_query(self, message: AgentMessage) -> AsyncGenerator[AgentMessage, None]:
        """
    ...
    async def _process_user_query(
        self, user_id: str, query_text: str, channel: str, language: str
    ) -> dict[str, Any]:
        """
    ...
    async def _search_knowledge_base(
        self, query_text: str, intent_analysis: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            for kb_id, kb_entry in self.knowledge_base.items():
                ...
                relevance_score = (
                    keyword_overlap + (2 if title_match else 0) + (1 if content_match else 0)
                )
                    ...
                    results.append(
                        {
                            ...
                            "content": (
                                kb_entry.get("content")[:500] + "..."
                                if len(kb_entry.get("content", "")) > 500
                                else kb_entry.get("content")
                            ),
    ...
    async def _generate_support_response(
        ...
        try:
            ...
            if intent == "technical_issue":
                response["text"] = (
                    "I'm sorry you're experiencing a technical issue. Let me help you troubleshoot this."
                )
            ...
            if kb_results:
                ...
                response["related_articles"] = [
                    {"title": kb["title"], "url": kb["url"]} for kb in kb_results[:3]
                ]
    ...
    async def _check_escalation_needed(
        ...
        try:
            ...
            # Negative sentiment
            if (
                sentiment_analysis.get("label") == "negative"
                and sentiment_analysis.get("score", 0) < -0.5
            ):
