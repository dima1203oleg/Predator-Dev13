...
class NaturalLanguageProcessingAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "nlp_agent", config: dict[str, Any] | None = None):
        super().__init__(agent_id, config or {})

        # NLP configuration
        self.nlp_config = {
            "supported_languages": self.config.get(
                "languages", ["en", "uk", "ru", "de", "fr", "es", "it", "pl", "cs"]
            ),
            "nlp_tasks": self.config.get(
                "tasks",
                [
                    "summarization",
                    "question_answering",
            ...
            "processing_limits": self.config.get(
                "limits",
                {
            ...
            "quality_settings": self.config.get(
                "quality",
                {
                    "min_confidence": 0.5,
    ...
    async def _initialize_nlp_models(self):
        ...
        try:
            ...
            self.nlp_pipelines["summarization"] = pipeline(
                "summarization", model=self.nlp_config["models"]["summarization"], device=device
            )

            # Question answering pipeline
            self.nlp_pipelines["qa"] = pipeline(
                "question-answering", model=self.nlp_config["models"]["qa"], device=device
            )
            ...
            self.nlp_pipelines["sentiment"] = pipeline(
                "sentiment-analysis", model=self.nlp_config["models"]["sentiment"], device=device
            )

            # Text generation
            self.nlp_pipelines["generation"] = pipeline(
                "text-generation", model=self.nlp_config["models"]["generation"], device=device
            )
    ...
    async def _handle_text_summarization(
        ...
        try:
            ...
            max_length = message.content.get(
                "max_length", self.nlp_config["processing_limits"]["max_summary_length"]
            )
    ...
    async def _summarize_text(
        self, text: str, max_length: int, min_length: int, language: str
    ) -> str:
        """
    ...
    async def _extractive_summarization(self, text: str, max_length: int, language: str) -> str:
        ...
        try:
            ...
            for i, sentence in enumerate(sentences):
                ...
                tfidf_score = sum(
                    self._calculate_word_importance(word, text) for word in words
                ) / len(words)
    ...
    async def _answer_questions(
        self, context: str, questions: list[str], language: str
    ) -> list[dict[str, Any]]:
        """
            ...
            for question in questions:
                try:
                    if "qa" in self.nlp_pipelines:
                        result = self.nlp_pipelines["qa"](question=question, context=context)
    ...
    async def _keyword_based_qa(self, question: str, context: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "confidence": (
                    min(max_score / len(question_keywords), 1.0) if question_keywords else 0.0
                ),
    ...
    async def _handle_entity_extraction(
        ...
        try:
            text = message.content.get("text", "")
            language = message.content.get("language", "en")
            entity_types = message.content.get(
    ...
    async def _analyze_sentiment(self, text: str, language: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"label": "neutral", "score": 0.5, "confidence": 0.0, "error": str(e)}
    ...
    def _rule_based_sentiment_uk(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "score": score,
                "confidence": min(
                    total_sentiment_words / 10, 1.0
                ),  # Confidence based on sentiment words
                "method": "rule_based_uk",
            }
    ...
    def _extract_keywords_rake(self, text: str, num_keywords: int) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for candidate, score in sorted_candidates[:num_keywords]:
                keywords.append({"keyword": candidate, "score": score, "method": "rake"})
    ...
    def _extract_keywords_position(self, text: str, num_keywords: int) -> list[dict[str, Any]]:
        ...
        try:
            ...
            important_sentences = (
                sentences[:2] + sentences[-2:] if len(sentences) > 2 else sentences
            )
            ...
            for sentence in important_sentences:
                ...
                for word in words:
                    ...
                    keywords.append(
                        {"keyword": word, "score": position_score, "method": "position"}
                    )
    ...
    async def _calculate_text_similarity(
        self, text1: str, text2: str, similarity_type: str
    ) -> float:
        """
    ...
    async def _generate_text(
        self, prompt: str, max_length: int, temperature: float, language: str
    ) -> str:
        """
    ...
    async def _handle_document_analysis(
        ...
        try:
            ...
            analysis_types = message.content.get(
                "analysis_types", ["summary", "entities", "sentiment", "keywords"]
            )
            language = message.content.get("language", "en")
    ...
    async def _analyze_document(
        self, document_text: str, analysis_types: list[str], language: str
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            for analysis_type in analysis_types:
                if analysis_type == "summary":
                    analysis["summary"] = await self._summarize_text(
                        document_text, 150, 50, language
                    )
        ...
        try:
            while True:
                try:
                    ...
                    expired_keys = [
                        key
                        for key, data in self.nlp_state["text_cache"].items()
                        if (current_time - data.get("timestamp", current_time)).seconds
                        > 7200  # 2 hours
                    ]
