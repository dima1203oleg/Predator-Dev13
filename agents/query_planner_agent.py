...
class QueryPlannerAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "query_planner_agent", config: dict[str, Any] | None = None):
        ...
        self.complexity_thresholds = {"simple": 1, "medium": 3, "complex": 5, "very_complex": 10}
        ...
        try:
            ...
            if not query:
                yield AgentMessage(
                    ...
                    from_agent=self.agent_id,
                    to_agent=message.from_agent,
                    content={"type": "error", "error": "Query required"},
                    timestamp=datetime.now(),
                )
                return
    ...
    async def _call_llm_for_analysis(self, prompt: str) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            if not query or not execution_plan:
                yield AgentMessage(
                    ...
                    from_agent=self.agent_id,
                    to_agent=message.from_agent,
                    content={"type": "error", "error": "Query and execution plan required"},
                    timestamp=datetime.now(),
                )
                return
    ...
    async def _execute_stage(
        self, stage_name: str, agents: list[str], query: str, parallel: bool, timeout: int
    ) -> dict[str, Any]:
        """
    ...
    async def _call_agent(
        self, agent_id: str, query: str, context: str, timeout: int
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            AgentMessage(
                ...
                from_agent=self.agent_id,
                to_agent=agent_id,
                content={"type": "process_query", "query": query, "context": context},
                timestamp=datetime.now(),
            )
        ...
        try:
            ...
            if successful_stages < total_stages:
                validation["issues"].append(
                    f"Only {successful_stages}/{total_stages} stages completed successfully"
                )
    ...
    def _track_query_performance(self, query: str, execution_result: dict[str, Any]):
        """
    ...
    async def _optimize_execution_plan(
        self, current_plan: dict[str, Any], performance_history: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
    ...
    async def _handle_performance_request(
        ...
        try:
            ...
            yield AgentMessage(
                ...
                from_agent=self.agent_id,
                to_agent=message.from_agent,
                content={"type": "performance_response", "metrics": performance_metrics},
                timestamp=datetime.now(),
            )
            ...
            if len(self.query_history) >= 20:
                ...
                metrics["performance_trends"] = {
                    "execution_time_change": (
                        (recent_avg_time - previous_avg_time) / previous_avg_time
                    )
                    * 100,
                    "trend_direction": (
                        "improving" if recent_avg_time < previous_avg_time else "degrading"
                    ),
                }
