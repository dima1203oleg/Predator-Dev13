...
from ..api.models import DeploymentHistory, InfrastructureStatus, MonitoringMetrics
...
class DeploymentAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.deployment_config = {
            ...
            "deployment_strategies": self.config.get("strategies", [
                "rolling_update", "blue_green", "canary", "a_b_testing"
            ]),
    ...
    async def start(self):
        ...
        self.infrastructure_monitor_task = asyncio.create_task(self._continuous_infrastructure_monitoring())
    ...
    async def _blue_green_deployment(
        ...
        try:
            ...
            if not test_result.get("passed", False):
                ...
                return {
                    "status": "failed",
                    "message": "Tests failed on green environment"
                }
    ...
    async def _switch_traffic_to_environment(
        self,
        app_name: str,
        environment: str
    ):
    ...
    async def _promote_canary_to_full(
        self,
        canary_app: str,
        main_app: str,
        environment: str
    ...
    async def _rollback_deployment(
        ...
        try:
            ...
            if target_version:
                ...
                previous_records = [
                    r for r in self.deployment_state["deployment_history"] 
    ...
    async def _handle_deployment_scaling(
        ...
        try:
            ...
            namespace = message.content.get("namespace", 
                self.deployment_config["kubernetes_config"]["namespace"])
    ...
    async def _execute_tekton_pipeline(
        self,
        pipeline_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            if self.k8s_client:
                ...
                healthy_nodes = sum(1 for node in nodes.items 
                    if all(condition.status == "True" for condition in node.status.conditions 
