...
from ..api.models import AnomalyDetection, DataStream, Alert
...
class AnomalyDetectionAgent(BaseAgent):
    ...
    Uses multiple detection methods: statistical, ML-based, and deep learning approaches
    """
    
    def __init__(
        self,
        agent_id: str = "anomaly_detection_agent",
        ...
        self.anomaly_config = {
            "detection_methods": self.config.get("detection_methods", [
                ...
                "statistical", "time_series", "autoencoder"
            ]),
            "contamination": self.config.get("contamination", 0.1),  # Expected anomaly ratio
            "window_size": self.config.get("window_size", 100),  # Sliding window size
            "threshold_sensitivity": self.config.get("threshold_sensitivity", 0.95),  # Detection sensitivity
            "alert_cooldown": self.config.get("alert_cooldown", 300),  # seconds between alerts
            "retraining_interval": self.config.get("retraining_interval", 3600),  # seconds
            ...
            while True:
                try:
                    ...
                    for stream_id in self.models:
                        if stream_id in self.data_buffers and len(self.data_buffers[stream_id]) >= 50:
    ...
    async def _monitor_stream_anomalies(self, stream_id: str):
        ...
        try:
            ...
            if ensemble_score > self.anomaly_config["threshold_sensitivity"]:
                await self._handle_anomaly_detection(stream_id, ensemble_score, anomaly_results)
    ...
    def _statistical_anomaly_detection(self, feature_matrix: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            for i in range(feature_matrix.shape[1]):
                ...
                IQR = Q3 - Q1
                iqr_anomalies = (feature_values < Q1 - 1.5 * IQR) | (feature_values > Q3 + 1.5 * IQR)
    ...
    def _time_series_anomaly_detection(self, feature_matrix: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            for i in range(feature_matrix.shape[1]):
                ...
                # Detect valleys
                valleys, _ = find_peaks(-feature_values, distance=5, prominence=np.std(feature_values))
    ...
    async def _handle_anomaly_detection(
        ...
        try:
            ...
            if last_alert and (datetime.now() - last_alert).seconds < self.anomaly_config["alert_cooldown"]:
                return
    ...
    async def _generate_anomaly_alert(self, anomaly_record: Dict[str, Any]):
        ...
        """
        try:
            alert_message = f"Anomaly detected in stream {anomaly_record['stream_id']} "
            alert_message += f"with score {anomaly_record['ensemble_score']:.3f} "
            alert_message += f"(severity: {anomaly_record['severity']})"
        ...
        try:
            ...
            for stream_id in self.anomaly_history:
                self.anomaly_history[stream_id] = [
                    anomaly for anomaly in self.anomaly_history[stream_id]
        ...
        try:
            ...
            yield AgentMessage(
                ...
                content={
                    ...
                    "ensemble_score": ensemble_score,
                    "method_results": anomaly_results,
                    "anomaly_detected": ensemble_score > self.anomaly_config["threshold_sensitivity"]
                },
                timestamp=datetime.now()
            )
        ...
        try:
            ...
            if len(self.data_buffers[stream_id]) >= 10:
                ...
                for method in ["statistical", "time_series"]:  # Quick methods for real-time
                    try:
                        if method == "statistical":
                            scores = self._statistical_anomaly_detection(feature_matrix[-10:])  # Last 10 points
                        elif method == "time_series":
                            scores = self._time_series_anomaly_detection(feature_matrix[-20:])  # Last 20 points
