...
from ..api.models import CustomsData, Company, CorruptionAlert
...
class CorruptionDetectorAgent(BaseAgent):
    ...
    def __init__(
        self,
        agent_id: str = "corruption_detector_agent",
        ...
        self.alert_severity_levels = self.config.get("alert_severity_levels", {
            "low": 0.6,
            "medium": 0.75,
            "high": 0.9,
            "critical": 0.95
    ...
    async def _handle_corruption_scan(
        ...
        try:
            ...
            saved_alerts = await self._save_corruption_alerts(alerts, message.content.get("user_id"))
        ...
        try:
            async with get_db_session() as session:
                ...
                query = session.query(
                    ...
                    Company, CustomsData.company_id == Company.id
                ).filter(
                    CustomsData.declaration_date.between(start_date, end_date)
                )
    ...
    def _rule_based_detection(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            df["company_age_days"] = (pd.Timestamp.now() - pd.to_datetime(df["company_registration_date"])).dt.days
    ...
    async def _ml_based_detection(
        ...
        try:
            ...
            # Get feature matrix
            feature_cols = [col for col in features_df.columns if col.startswith(('value_', 'freq_', 'ratio_', 'age_'))]
                ...
                if confidence >= min_confidence:
                    ...
                    alerts.append({
                        ...
                        "evidence": {
                            ...
                            "features": {col: row[col] for col in feature_cols[:5]}  # Top 5 features
                        }
                    })
    ...
    def _prepare_ml_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Time-based features
            features_df["declaration_hour"] = pd.to_datetime(features_df["declaration_date"]).dt.hour
            features_df["is_business_hours"] = features_df["declaration_hour"].between(9, 17).astype(int)
            
            # Company age features
            features_df["company_age_days"] = (pd.Timestamp.now() - pd.to_datetime(features_df["company_registration_date"])).dt.days
            ...
            features_df["value_to_hs_mean_ratio"] = features_df["value_usd"] / features_df["hs_code_mean"]
            
            # Country risk features (simplified)
            high_risk_countries = ["RU", "CN", "IR", "KP", "SY"]  # Example high-risk countries
            features_df["high_risk_country"] = features_df["country_origin"].isin(high_risk_countries).astype(int)
    ...
    async def _get_or_train_model(
        ...
        try:
            ...
            if (self.last_model_update and
                (datetime.now() - self.last_model_update).days < self.model_update_frequency and
                model_key in self.models_cache):
                return self.models_cache[model_key]
            
            # Prepare training data
            feature_cols = [col for col in features_df.columns if col.startswith(('value_', 'freq_', 'ratio_', 'age_', 'is_', 'high_'))]
            X = features_df[feature_cols].fillna(0)
    ...
    async def _analyze_corruption_alert(self, alert_id: str) -> Dict[str, Any]:
        ...
        try:
            async with get_db_session() as session:
                ...
                if alert.company_id:
                    related_query = session.query(CorruptionAlert).filter(
                        CorruptionAlert.company_id == alert.company_id,
                        CorruptionAlert.id != alert.id
                    ).order_by(CorruptionAlert.created_at.desc()).limit(10)
    ...
    async def _analyze_company(self, company_id: str) -> Dict[str, Any]:
        ...
        try:
            async with get_db_session() as session:
                ...
                risk_score = min(100, len(alerts) * 10 + (total_declarations / 10))  # Simple risk scoring
    ...
    async def _assess_company_risk(
        ...
        try:
            ...
            risk_factors = {
                "alert_frequency": len(recent_alerts) / (assessment_period_days / 30),  # Alerts per month
                "high_severity_alerts": len([a for a in recent_alerts if a.severity in ["high", "critical"]]),
                "company_age_risk": 1.0 if company_analysis.get("company_age_days", 365) < 365 else 0.0,
                "activity_volume_risk": min(1.0, company_analysis.get("total_declarations", 0) / 1000),  # Scale with volume
            }
