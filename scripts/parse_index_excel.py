    ...
    try:
        ...
        if hasattr(info, "config") and hasattr(info.config, "params"):
            try:
                ...
                got = None
        if got and got != dim:
            qd.recreate_collection(collection_name=name, vectors_config=qm.VectorParams(size=dim, distance=qm.Distance.COSINE))
    except Exception:
        qd.recreate_collection(collection_name=name, vectors_config=qm.VectorParams(size=dim, distance=qm.Distance.COSINE))
...
def main():
    ...
    rows: T.List[dict] = df.to_dict(orient="records")
    if args.verbose:
    ...
    if not args.dry_run:
        with eng.begin() as cx:
            for r in rows:
                ...
                pk = sha256(payload)
                op_hash = sha256(str(r.get("id","")) + "|" + str(r.get("date","")) + "|" + str(r.get("amount","")))
                cx.exec_driver_sql(
    ...
    os_client = OpenSearch(hosts=[args.opensearch], http_compress=True, use_ssl=False, verify_certs=False)
    ...
    # Пакетні ембеддинги
    vectors: T.List[T.List[float]] = emb_client.embed_list(texts, batch=args.embed_batch)
    ...
    print(f"OK: indexed PG(+), OS[{args.index}], Qdrant[{args.collection}] with dim={dim}, model={args.embed_model}")

def run_pipeline(input_path: str, db: str | None, opensearch: str | None, qdrant: str | None, minio: str | None, bucket: str, embed_model: str = "nomic-embed-text", embed_batch: int = 64, dry_run: bool = False, verbose: bool = False) -> dict:
    """
    ...
    result = {"rows_parsed": len(rows), "pg_upserted": 0, "os_indexed": 0, "qdrant_indexed": 0, "errors": []}
    ...
    # OpenSearch & Qdrant клієнти
    try:
        os_client = OpenSearch(hosts=[opensearch], http_compress=True, use_ssl=False, verify_certs=False)
        ...
        # Пакетні ембеддинги
        vectors: T.List[T.List[float]] = emb_client.embed_list(texts, batch=embed_batch)
        ...
        if verbose:
            print(f"OK: indexed PG(+), OS[pa-dataset-v1], Qdrant[pa_domain_v1] with dim={dim}, model={embed_model}")
    except Exception as e:
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Parse an Excel and optionally index to DB / search backends")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Do not write to external systems; show actions")
    parser.add_argument("--mock", action="store_true", help="Use internal mocks instead of real clients")
    ...
    if args.dry_run:
        ...
        print("  -> implement DB client and indexing logic, or run with --mock to use internal mock")
