...
class OllamaEmbClient:
    ...
    def embed_once(self, text: str) -> T.List[float]:
        r = self._http.post(f"{self.base_url}/api/embeddings", json={"model": self.model, "prompt": text})
        r.raise_for_status()
        data = r.json()
    ...
    def embed_list(self, texts: T.List[str], batch: int = 64) -> T.List[T.List[float]]:
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_fixed(1))
    def tags(self) -> T.Dict:
        r = self._http.get(f"{self.base_url}/api/tags")
        r.raise_for_status()
        return r.json()

def infer_dim(embedding: T.List[float]) -> int:
